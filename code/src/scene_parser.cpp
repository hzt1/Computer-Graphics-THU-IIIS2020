...
void SceneParser::parsePerspectiveCamera() {
    ...
    if (!strcmp(token, "focal")) {
        ...
        getToken(token);
        camera = new PerspectiveCamera(center, direction, up, width, height, angle_radians, focal);
    }
    assert (!strcmp(token, "}"));
    camera = new PerspectiveCamera(center, direction, up, width, height, angle_radians, 1e9);
}
...
Plane *SceneParser::parsePlane() {
    ...
    if (!strcmp(token, "texture")) {
        ...
        getToken(token);
        assert (!strcmp(token, "x-axis"));
        xdir = readVector3f();
        getToken(token);
        assert (!strcmp(token, "y-axis"));
        ydir = readVector3f();
        getToken(token);
        assert (!strcmp(token, "x-offset"));
        xoff = readFloat();
        getToken(token);
        assert (!strcmp(token, "y-offset"));
        yoff = readFloat();
        getToken(token);
    }
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    return new Plane(normal, offset, current_material, image, xdir, ydir, xoff, yoff);
}
...
Mesh *SceneParser::parseTriangleMesh() {
    ...
    Mesh *answer = new Mesh(filename, current_material, scale, offset, inv);
